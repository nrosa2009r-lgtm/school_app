...
def _get_school_cutoff(school_id: Optional[int] = None) -> tuple[int, int]:
    """Pobiera godzinę graniczną dla szkoły z cache'a lub konfiguracji."""
    if school_id:
        try:
            from config.conf import get_data
            # Najpierw sprawdź w konfiguracji
            cutoff_h = get_data(f"SCHOOL_{school_id}_CUTOFF_HOUR")
            if cutoff_h and str(cutoff_h).isdigit():
                return int(cutoff_h), 0
        except Exception:
            pass
    # Fallback: globalna konfiguracja
    try:
        from config.conf import get_data
        global_cut = get_data("SCHOOL_CUTOFF_HOUR")
        if global_cut and str(global_cut).isdigit():
            return int(global_cut), 0
    except Exception:
        pass
    return CUTOFF_HOUR, CUTOFF_MINUTE


def is_past_cutoff(now: Optional[datetime] = None, school_id: Optional[int] = None, order_date: Optional[str] = None) -> bool:
    """
    Sprawdza czy minęła godzina graniczna.
    Jeśli order_date jest na przyszłość (jutro/dalej), NIE blokuje.
    """
    if now is None:
        now = datetime.now()

    # Klauzula wyprzedzenia: zamówienia na przyszłe dni nie są blokowane przez dzisiejsze odcięcie
    if order_date:
        today_str = now.strftime("%Y-%m-%d")
        if order_date > today_str:
            return False

    ch, cm = _get_school_cutoff(school_id)
    return now.time() >= dt_time(ch, cm)
